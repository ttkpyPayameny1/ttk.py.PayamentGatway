...
# CONFIG
# =========================
BOT_TOKEN = "8983512458:AAFn53mUa_zEqa3taCfD37grYC02MkyFBHA"  # Replace with actual token
ADMIN_ID = 2037461288
OWNER_ID = ADMIN_ID
...
def init_db():
    ...
    cur.execute(
        "CREATE TABLE IF NOT EXISTS admins (user_id INTEGER PRIMARY KEY, role"
        " TEXT DEFAULT 'admin', added_at TEXT)"
    )
    cur.execute(
        "INSERT OR IGNORE INTO admins(user_id,role,added_at)"
        " VALUES(?,?,?)",
        (ADMIN_ID, "owner", now_iso()),
    )
    cur.execute(
...
def main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="🔐 ফান্ড ডিপোজিট"),
                KeyboardButton(text="👤 একাউন্ট"),
            ],
            [
            ...
            [
                KeyboardButton(text="📊 Daily Earnings"),
                KeyboardButton(text="🎁 Bonus Center"),
            ],
            [
                KeyboardButton(text="💎 Available Plans"),
...
async def cancel(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(
        "❌ বর্তমান প্রক্রিয়াটি বাতিল করা হয়েছে।", reply_markup=main_keyboard()
    )
...
async def deposit_menu(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="🟡 Binance", callback_data="dep_binance"
                )
            ],
        ]
    )
...
async def dep_local_amount(message: Message, state: FSMContext):
    ...
    if amount < 50 or amount > 10000:
        await message.answer(
            "⚠️ ডিপোজিটের পরিমাণ 50৳ থেকে 10,000৳ এর মধ্যে হতে হবে।"
        )
        return
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💳 পেমেন্ট গেটওয়েতে যান", url=PAYMENT_GATEWAY_URL
                )
            ],
            [
                InlineKeyboardButton(
                    text="❌ Cancel", callback_data="cancel_inline"
                )
            ],
        ]
    )
    await message.answer(
        ...
        "আপনার প্রদত্ত এমাউন্টটি ডিপোজিট করতে নিচের দেওয়া লিংকে প্রবেশ"
        " করুন。\n\n"
        f"💳 Method: {method}",
        reply_markup=kb,
    )
...
async def send_admin_deposit(user, method, amount, txid, oid):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Approve", callback_data=f"adm_dep_ok:{oid}"
                ),
                InlineKeyboardButton(
                    text="❌ Reject", callback_data=f"adm_dep_no:{oid}"
                ),
            ],
        ]
    )
...
async def wd_amount(message: Message, state: FSMContext):
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE users SET main_balance=main_balance-? WHERE user_id=? AND"
        " main_balance>=?",
    ...
    if cur.rowcount != 1:
        ...
        con.close()
        await message.answer(
            "❌ ব্যালেন্স পরিবর্তিত হয়েছে। আবার চেষ্টা করুন।"
        )
        return
    ...
    await state.clear()
    await message.answer(
        f"✅ আপনার 💸 {money(amount)}৳ উইথড্র রিকোয়েস্ট সফলভাবে জমা"
        " হয়েছে!\n\n"
    ...
    await send_admin_withdraw(
        message.from_user, wid, method, account_no, amount
    )
...
async def send_admin_withdraw(user, wid, method, account_no, amount):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Approve", callback_data=f"adm_wd_ok:{wid}"
                ),
                InlineKeyboardButton(
                    text="❌ Reject", callback_data=f"adm_wd_no:{wid}"
                ),
            ]
        ]
    )
...
async def ref_rules(call: CallbackQuery):
    await call.message.answer(
        "📋 Referral Rules\n\n"
        "রেফার করা ব্যক্তি ডিপোজিট করলে তার ডিপোজিটের ৫% কমিশন Systems"
        " নির্ধারিত নিয়ম অনুযায়ী Referral Balance-এ যোগ হতে পারে।"
    )
    await call.answer()
...
async def daily_earnings(message: Message):
    ...
    lines.append(
        "⚠️ Claim না করলে ওই দিনের amount পরের দিনে carry হবে না।"
    )
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🧮 Today Earnings Claim", callback_data="claim_today"
                )
            ]
        ]
    )
...
async def claim_today(call: CallbackQuery):
    ...
    if now.hour < 8:
        await call.answer(
            "⏳ Claim window সকাল ৮টা থেকে শুরু হবে।", show_alert=True
        )
        return
    ...
    if total == 0:
        await call.answer(
            "⚠️ আজকের earnings ইতিমধ্যে claim করা হয়েছে বা available নেই।",
            show_alert=True,
        )
        return
    ...
    await call.message.answer(
        ...
        "ℹ️ এটি DEMO/SIMULATION calculation; কোনো নির্দিষ্ট লাভ নিশ্চিত"
        " নয়।"
    )
...
async def bonus_center(message: Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🎁 Daily Free 5৳ Claim", callback_data="bonus_claim"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📜 Bonus History", callback_data="bonus_history"
                )
            ],
        ]
    )
    await message.answer(
        ...
        "📌 Bonus Balance সরাসরি withdraw বা plan purchase-এর জন্য ব্যবহার"
        " করা যাবে না।\n"
...
async def bonus_claim(call: CallbackQuery):
    ...
    cur.execute(
        "UPDATE users SET bonus_balance=bonus_balance+5 WHERE user_id=?", (uid,)
    )
    cur.execute(
        """
...
async def plan_details(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💸 Plan Buy", callback_data=f"buy:{plan}"
                )
            ],
    ...
    await call.message.answer(
        ...
        "⚠️ এটি শুধুমাত্র DEMO/SIMULATION calculation; কোনো নির্দিষ্ট"
        " লাভ নিশ্চিত নয়।",
        reply_markup=kb,
    )
    await call.answer()
...
async def buy_plan(call: CallbackQuery):
    ...
    if active_count >= 2:
        ...
        await call.message.answer(
            ...
            "💎 তবে অন্যান্য Available Plans থেকে Plan নির্বাচন করতে"
            " পারবেন।"
        )
        await call.answer()
        return
...
async def show_help(message: Message):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📞 Support Center", url="https://t.me/Nikan_Support01"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📢 Official Channel", url="https://t.me/NIKAN_EARN"
                )
            ],
            [
                InlineKeyboardButton(
...
async def admin_dep_reject(call: CallbackQuery):
    ...
    await bot.send_message(
        uid,
        "❌ Deposit Failed!\n\n"
        "আপনার দেওয়া Transaction ID সঠিক নয় অথবা এই Transaction ID-এর মাধ্যমে"
        " কোনো পেমেন্ট শনাক্ত করা যায়নি।\n\n"
        f"📝 Transaction ID: {txid}\n"
        f"🆔 Order No: {oid}\n\n"
...
async def admin_wd_reject(call: CallbackQuery):
    ...
    await bot.send_message(
        ...
        f"↩️ {money(amount)}৳ আপনার অ্যাকাউন্ট ব্যালেন্সে পুনরায় যোগ"
        " হয়েছে।\n\n"
...
async def inline_cancel(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.answer(
        "❌ বাতিল করা হয়েছে।", reply_markup=main_keyboard()
    )
    await call.answer()
...
def setset(k, v):
    con = db()
    con.execute(
        "INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO"
...
def role(uid):
    con = db()
    r = con.execute(
        "SELECT role FROM admins WHERE user_id=?", (uid,)
    ).fetchone()
    con.close()
...
def admin_kb(uid):
    rows = [
        ...
        [
            InlineKeyboardButton(
                text="⚙️ Bot Controls", callback_data="x:controls"
            ),
            InlineKeyboardButton(
                text="💳 Payment Controls", callback_data="x:pay"
            ),
        ],
        [
            InlineKeyboardButton(text="📈 Limits", callback_data="x:limits"),
            InlineKeyboardButton(
                text="📣 Force Join", callback_data="x:force"
            ),
        ],
        [
            InlineKeyboardButton(
                text="👤 User Control", callback_data="x:user"
            ),
            InlineKeyboardButton(text="📥 Deposits", callback_data="x:deps"),
        ],
        [
            InlineKeyboardButton(text="💸 Withdraws", callback_data="x:wds"),
            InlineKeyboardButton(
                text="📢 Broadcast", callback_data="x:broadcast"
            ),
        ],
        [
            InlineKeyboardButton(
                text="✉️ User Broadcast", callback_data="x:ubroadcast"
            )
        ],
    ]
    if super_ok(uid):
        rows.append([
            InlineKeyboardButton(
                text="👑 Admin Management", callback_data="x:admins"
            )
        ])
    rows.append(
        [InlineKeyboardButton(text="⬅️ Main Menu", callback_data="x:close")]
    )
...
async def x_dash(call: CallbackQuery):
    ...
    vip = (
        cur.execute("SELECT COUNT(DISTINCT user_id) FROM plans").fetchone()[0]
        if cur.execute(
    ...
    fund = cur.execute(
        "SELECT COALESCE(SUM(amount),0) FROM deposits WHERE status"
        " IN ('Approved','Success')"
    ).fetchone()[0]
    wth = cur.execute(
        "SELECT COALESCE(SUM(amount),0) FROM withdrawals WHERE status"
        " IN ('Approved','Success')"
    ).fetchone()[0]
    con.close()
    await call.message.edit_text(
        ...
        f" Total Withdraw: {money(wth)}৳\n\n🤖 Bot:"
        f' {"🟢 ON" if getset("bot_enabled")=="1" else "🔴 OFF"}',
        reply_markup=admin_kb(call.from_user.id),
    )
    await call.answer()
...
async def x_controls(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=a_toggle("🤖 Bot", "bot_enabled"),
                    callback_data="x:t:bot_enabled",
                )
            ],
            [
                InlineKeyboardButton(
                    text=a_toggle("💰 Fund", "fund_enabled"),
                    callback_data="x:t:fund_enabled",
                )
            ],
            [
                InlineKeyboardButton(
                    text=a_toggle("💸 Withdraw", "withdraw_enabled"),
                    callback_data="x:t:withdraw_enabled",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⬅️ Back", callback_data="x:open"
                )
            ],
        ]
    )
...
async def x_pay(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=a_toggle("Dep bKash", "dep_bkash_enabled"),
                    callback_data="x:t:dep_bkash_enabled",
                ),
                InlineKeyboardButton(
                    text=a_toggle("Dep Nagad", "dep_nagad_enabled"),
                    callback_data="x:t:dep_nagad_enabled",
                ),
            ],
            [
                InlineKeyboardButton(
                    text=a_toggle("Wth bKash", "wth_bkash_enabled"),
                    callback_data="x:t:wth_bkash_enabled",
                ),
                InlineKeyboardButton(
                    text=a_toggle("Wth Nagad", "wth_nagad_enabled"),
                    callback_data="x:t:wth_nagad_enabled",
                ),
            ],
            [
                InlineKeyboardButton(
                    text=a_toggle("Wth Rocket", "wth_rocket_enabled"),
                    callback_data="x:t:wth_rocket_enabled",
                ),
                InlineKeyboardButton(
                    text=a_toggle("Wth Upay", "wth_upay_enabled"),
                    callback_data="x:t:wth_upay_enabled",
                ),
            ],
            [
                InlineKeyboardButton(
                    text="🔄 All Withdraw ON/OFF", callback_data="x:allw"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📲 Set bKash Number", callback_data="x:num:bKash"
                ),
                InlineKeyboardButton(
                    text="📲 Set Nagad Number", callback_data="x:num:Nagad"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="🔗 Add/Edit bKash Gateway", callback_data="x:gw:bKash"
                ),
                InlineKeyboardButton(
                    text="🔗 Add/Edit Nagad Gateway", callback_data="x:gw:Nagad"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="🗑 Remove bKash Gateway", callback_data="x:rm:bKash"
                ),
                InlineKeyboardButton(
                    text="🗑 Remove Nagad Gateway", callback_data="x:rm:Nagad"
                ),
            ],
            [
                InlineKeyboardButton(
    ...
    await call.message.edit_text(
        "💳 PAYMENT CONTROLS\n━━━━━━━━━━━━━━━━━━\nবিকাশ/নগদের Payment"
        " Gateway link এখান থেকেই Add/Edit/Remove করা যাবে।",
        reply_markup=kb,
    )
    await call.answer()
...
async def x_num(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"✅ {m} number updated: {n}", reply_markup=main_keyboard()
    )
...
async def x_gw(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"✅ {m} gateway saved.", reply_markup=main_keyboard())
...
async def x_limits(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f'Min Dep {getset("min_dep")}৳',
                    callback_data="x:v:min_dep",
                ),
                InlineKeyboardButton(
                    text=f'Max Dep {getset("max_dep")}৳',
                    callback_data="x:v:max_dep",
                ),
            ],
            [
                InlineKeyboardButton(
                    text=f'Min Wth {getset("min_wth")}৳',
                    callback_data="x:v:min_wth",
                ),
                InlineKeyboardButton(
                    text=f'Max Wth {getset("max_wth")}৳',
                    callback_data="x:v:max_wth",
                ),
            ],
            [
                InlineKeyboardButton(
                    text=f'Daily Dep Limit {getset("daily_dep_limit")}',
                    callback_data="x:v:daily_dep_limit",
                )
            ],
            [
                InlineKeyboardButton(
    ...
    await call.message.edit_text(
        "📈 LIMIT & AMOUNT CONFIGURATION", reply_markup=kb
    )
    await call.answer()
...
async def x_value(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"✅ {k} = {money(v)} সেট হয়েছে।", reply_markup=main_keyboard()
    )
...
async def x_force(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=a_toggle("📣 Force Join", "force_join_enabled"),
                    callback_data="x:tforce",
                )
            ],
            [
                InlineKeyboardButton(
                    text="➕ Set Channel/Group", callback_data="x:fjset"
                ),
                InlineKeyboardButton(text="🗑 Remove", callback_data="x:fjrm"),
            ],
            [
                InlineKeyboardButton(
                    text="⬅️ Back", callback_data="x:open"
    ...
    await call.message.edit_text(
        ...
        f' {getset("force_join_link") or "Not set"}',
        reply_markup=kb,
    )
    await call.answer()
...
async def x_tforce(call: CallbackQuery):
    if not admin_ok(call.from_user.id):
        return await call.answer("Admin only", show_alert=True)
    setset(
        "force_join_enabled", "0" if getset("force_join_enabled") == "1" else "1"
    )
    await x_force(call)
...
async def x_fjset(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "📣 Channel username বা chat ID পাঠান। যেমন @NIKAN_EARN"
    )
    await call.answer()
...
async def x_fjlink(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        "✅ Force Join set ও ON হয়েছে।", reply_markup=main_keyboard()
    )
...
async def x_user(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔎 Search User by ID", callback_data="x:usearch"
                )
            ],
            [
                InlineKeyboardButton(
                    text="💰 Edit Balances / Referral Count",
                    callback_data="x:usearch",
                )
            ],
            [
                InlineKeyboardButton(
                    text="💎 Plan Add", callback_data="x:plan"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    await call.message.edit_text(
        "👤 USER CONTROL PANEL\n\nUser ID দিয়ে search করে balance, referral,"
        " ban/suspend ও plan control করুন।",
        reply_markup=kb,
    )
    await call.answer()
...
async def x_userid(message: Message, state: FSMContext):
    ...
    if not r:
        ...
        return await message.answer(
            "❌ User পাওয়া যায়নি।", reply_markup=main_keyboard()
        )
    await state.clear()
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🚫 Ban", callback_data=f"x:ban:{uid}:1"
                ),
                InlineKeyboardButton(
                    text="✅ Unban", callback_data=f"x:ban:{uid}:0"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="⛔ Suspend", callback_data=f"x:sus:{uid}:1"
                ),
                InlineKeyboardButton(
                    text="🟢 Unsuspend", callback_data=f"x:sus:{uid}:0"
                ),
            ],
            [
                InlineKeyboardButton(
                    text="💰 Deposit Balance",
                    callback_data=f"x:edit:{uid}:deposit_balance",
                ),
                InlineKeyboardButton(
                    text="🫂 Referral Balance",
                    callback_data=f"x:edit:{uid}:referral_balance",
                ),
            ],
            [
                InlineKeyboardButton(
                    text="🎁 Bonus Balance",
                    callback_data=f"x:edit:{uid}:bonus_balance",
                ),
                InlineKeyboardButton(
                    text="📝 Task Balance",
                    callback_data=f"x:edit:{uid}:task_balance",
                ),
            ],
            [
                InlineKeyboardButton(
                    text="💵 Main Balance",
                    callback_data=f"x:edit:{uid}:main_balance",
                ),
                InlineKeyboardButton(
                    text="👥 Referral Count",
                    callback_data=f"x:edit:{uid}:referral_count",
                ),
            ],
            [
                InlineKeyboardButton(
                    text="🎁 Bonus → Main", callback_data=f"x:bmain:{uid}"
                )
            ],
        ]
    )
    await message.answer(
        f"👤 USER\n━━━━━━━━━━━━━━━━━━\n👤 {r[1]}\n🔗 @{r[2] or 'None'}\n🆔"
        f" {r[0]}\n💵 Main: {money(r[3])}৳\n📥 Deposit: {money(r[4])}৳\n🫂"
        f" Referral: {money(r[6])}৳\n🎁 Bonus: {money(r[5])}৳\n📝 Task:"
        f" {money(r[9])}৳\n👥 Referrals: {r[10]}\n🚫 Ban:"
        f' {"YES" if r[12] else "NO"}\n⛔ Suspend: {"YES" if r[13] else "NO"}',
        reply_markup=kb,
    )
...
async def x_edit_value(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"✅ User {uid}: {field} = {money(v)}", reply_markup=main_keyboard()
    )
...
async def x_bmain_prompt(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdvancedAdminState.user_value)
    await call.message.answer(
        "🎁 Bonus Balance থেকে কত টাকা Main Balance-এ transfer করবেন?"
    )
    await call.answer()
...
async def x_plan_uid(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=p, callback_data=f"x:padd:{uid}:{p}")
                for p in names[i : i + 3]
            ]
            for i in range(0, len(names), 3)
        ]
...
async def x_plan_add(call: CallbackQuery, state: FSMContext):
    ...
    cur.execute(
        "INSERT INTO"
        " plans(user_id,plan,amount,activated_at,expires_at,active)"
        " VALUES(?,?,?,?,?,1)",
        (
            uid,
            p,
...
async def x_broadcast(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "📢 Broadcast message লিখুন। তারপর Confirm/Cancel দেখানো হবে।"
    )
    await call.answer()
...
async def x_bcok(call: CallbackQuery, state: FSMContext):
    ...
    ids = [
        ...
        for r in con.execute(
            "SELECT user_id FROM users WHERE banned=0 AND suspended=0"
        )
    ]
    con.close()
    sent = fail = 0
...
async def x_deps(call: CallbackQuery):
    ...
    con = db()
    rows = con.execute(
        "SELECT id,user_id,method,amount,txid,order_no,status FROM deposits"
        " WHERE status IN ('Pending','Processing') ORDER BY id DESC LIMIT 10"
    ).fetchall()
    con.close()
    if not rows:
    ...
    for did, uid, m, a, t, o, s in rows:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⚙️ Processing", callback_data=f"x:dproc:{did}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="✅ Approve", callback_data=f"x:dok:{o}"
                    ),
                    InlineKeyboardButton(
                        text="❌ Reject", callback_data=f"x:dno:{o}"
                    ),
                ],
            ]
        )
...
async def x_dproc(call: CallbackQuery):
    ...
    con = db()
    con.execute(
        "UPDATE deposits SET status='Processing',updated_at=? WHERE id=?",
        (now_iso(), did),
    )
    con.commit()
    con.close()
...
async def x_dok(call: CallbackQuery):
    ...
    cur = con.cursor()
    r = cur.execute(
        "SELECT id,user_id,method,amount,txid,status FROM deposits WHERE"
        " order_no=?",
        (oid,),
    ).fetchone()
    if not r:
    ...
    cur.execute(
        "UPDATE deposits SET status='Approved',reason='Approved by"
    ...
    await bot.send_message(
        ...
        f" Transaction ID: {t}\n🆔 Order Number: {oid}\n━━━━━━━━━━━━━━━━━━\n💳"
        f" আপনার মূল ডিপোজিট ব্যালেন্সে {money(a)} BDT যোগ করা হয়েছে।\nধন্যবাদ।"
...
async def x_wds(call: CallbackQuery):
    ...
    con = db()
    rows = con.execute(
        "SELECT id,user_id,method,account,amount,status FROM withdrawals"
        " WHERE status IN ('Pending','Processing') ORDER BY id DESC LIMIT 10"
    ).fetchall()
    con.close()
    if not rows:
    ...
    for wid, uid, m, acc, a, s in rows:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⚙️ Processing", callback_data=f"x:wproc:{wid}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="✅ Approve", callback_data=f"x:wok:{wid}"
                    ),
                    InlineKeyboardButton(
                        text="❌ Reject", callback_data=f"x:wno:{wid}"
                    ),
                ],
            ]
        )
...
async def x_wproc(call: CallbackQuery):
    ...
    con = db()
    con.execute(
        "UPDATE withdrawals SET status='Processing',updated_at=? WHERE id=?",
        (now_iso(), wid),
    )
    con.commit()
    con.close()
...
async def x_wok(call: CallbackQuery):
    ...
    con = db()
    r = con.execute(
        "SELECT user_id,method,account,amount,status FROM withdrawals WHERE"
        " id=?",
        (wid,),
    ).fetchone()
    if not r:
    ...
    if s == "Approved":
        ...
        return await call.answer("Already approved", show_alert=True)
    con.execute(
        "UPDATE withdrawals SET status='Approved',reason='Approved by"
        " admin',updated_at=? WHERE id=?",
        (now_iso(), wid),
    )
    con.commit()
...
async def x_reject_reason(message: Message, state: FSMContext):
    ...
    if kind == "deposit":
        r = cur.execute(
            "SELECT user_id,amount,txid,method,status FROM deposits WHERE"
            " order_no=?",
            (rid,),
        ).fetchone()
        if not r:
        ...
        uid, a, t, m, s = r
        cur.execute(
            "UPDATE deposits SET status='Rejected',reason=?,updated_at=? WHERE"
            " order_no=?",
            (reason, now_iso(), rid),
        )
        await bot.send_message(
            uid,
            "❌ Deposit Failed!\nআপনার দেওয়া Transaction ID সঠিক নয় অথবা এই"
            f" Transaction ID-এর মাধ্যমে কোনো পেমেন্ট শনাক্ত করা যায়নি।\n📝"
            f" Transaction ID: {t}\n🆔 Order No: {rid}\n\n❗ Reject Reason:"
            f" {reason}\n\n🔄 অনুগ্রহ করে সঠিক Transaction ID দিয়ে আবার চেষ্টা"
            " করুন।",
        )
    else:
        r = cur.execute(
            "SELECT user_id,amount,method,account,status FROM withdrawals"
            " WHERE id=?",
            (int(rid),),
        ).fetchone()
        if not r:
        ...
        if s != "Rejected":
            cur.execute(
                "UPDATE withdrawals SET status='Rejected',reason=?,updated_at=?"
                " WHERE id=?",
                (reason, now_iso(), int(rid)),
            )
            cur.execute(
                "UPDATE users SET main_balance=main_balance+? WHERE user_id=?",
                (a, uid),
        ...
        await bot.send_message(
            ...
            f" রিকোয়েস্টটি বাতিল করা হয়েছে।\n↩️ {money(a)}৳ আপনার অ্যাকাউন্ট"
            f" ব্যালেন্সে পুনরায় যোগ হয়েছে।\n\n❗ Reject Reason: {reason}",
        )
    con.commit()
    con.close()
    await state.clear()
    await message.answer(
        "✅ Status updated এবং user notification পাঠানো হয়েছে।",
        reply_markup=main_keyboard(),
    )
...
async def x_admins(call: CallbackQuery):
    ...
    con = db()
    rows = con.execute(
        "SELECT user_id,role FROM admins ORDER BY user_id"
    ).fetchall()
    con.close()
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ Add Admin", callback_data="x:addadmin"
                ),
                InlineKeyboardButton(
                    text="➖ Remove Admin", callback_data="x:rmadmin"
                ),
            ],
            [
                InlineKeyboardButton(
...
# CONFIG
# =========================
BOT_TOKEN = "PUT_YOUR_BOT_TOKEN_HERE"  # আপনার বটের আসল টোকেন এখানে দিন
ADMIN_ID = 2037461288

PAYMENT_GATEWAY_URL = "[https://example.com/payment-gateway](https://example.com/payment-gateway)"
BINANCE_DEPOSIT_ADDRESS = "0xade76b7f023c3ded14850293ea26e477edbdd048"
USDT_RATE_BDT = Decimal("125")
...
async def referral(message: Message):
    ...
    me = await bot.get_me()
    link = f"[https://t.me/](https://t.me/){me.username}?start=ref_{uid}"
...
async def show_help(message: Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📞 Support Center", url="[https://t.me/Nikan_Support01](https://t.me/Nikan_Support01)")],
        [InlineKeyboardButton(text="📢 Official Channel", url="[https://t.me/NIKAN_EARN](https://t.me/NIKAN_EARN)")],
...
async def x_dash(call: CallbackQuery):
    ...
    con.close()
    await call.message.edit_text(f'📊 LIVE ADMIN DASHBOARD\n━━━━━━━━━━━━━━━━━━\n👥 Total Users: {users}\n💎 VIP Buyers: {vip}\n💰 Total Fund: {money(fund)}৳\n💸 Total Withdraw: {money(wth)}৳\n\n🤖 Bot: {"🟢 ON" if getset("bot_enabled")=="1" else "🔴 OFF"}', reply_markup=admin_kb(call.from_user.id))
    await call.answer()
...
async def x_num(message: Message, state: FSMContext):
    ...
    setset('bkash_number' if m == 'bKash' else 'nagad_number', n)
    await state.clear()
    await message.answer(f'✅ {m} number updated: {n}', reply_markup=main_keyboard())
...
async def x_value(message: Message, state: FSMContext):
    ...
    setset(k, money(v))
    await state.clear()
    await message.answer(f'✅ {k} = {money(v)} সেট হয়েছে।', reply_markup=main_keyboard())
...
async def x_force(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
      ...
      [InlineKeyboardButton(text='⬅️ Back', callback_data='x:open')]])
    await call.message.edit_text(f'📣 FORCE JOIN\nChat: {getset("force_join_chat") or "Not set"}\nLink: {getset("force_join_link") or "Not set"}', reply_markup=kb)
    await call.answer()
...
async def x_fjlink(message: Message, state: FSMContext):
    ...
    setset('force_join_enabled', '1')
    await state.clear()
    await message.answer('✅ Force Join set ও ON হয়েছে।', reply_markup=main_keyboard())
...
async def x_userid(message: Message, state: FSMContext):
    ...
    if not r: 
        await state.clear()
        return await message.answer('❌ User পাওয়া যায়নি।', reply_markup=main_keyboard())
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer(f'👤 USER\n━━━━━━━━━━━━━━━━━━\n👤 {r[1]}\n🔗 @{r[2] or "None"}\n🆔 {r[0]}\n💵 Main: {money(r[3])}৳\n📥 Deposit: {money(r[4])}৳\n🫂 Referral: {money(r[6])}৳\n🎁 Bonus: {money(r[5])}৳\n📝 Task: {money(r[9])}৳\n👥 Referrals: {r[10]}\n🚫 Ban: {"YES" if r[12] else "NO"}\n⛔ Suspend: {"YES" if r[13] else "NO"}', reply_markup=kb)
...
async def x_edit_value(message: Message, state: FSMContext):
    ...
    con.close()
    await state.clear()
    await message.answer(f'✅ User {uid}: {field} = {money(v)}', reply_markup=main_keyboard())
...
async def x_reject_reason(message: Message, state: FSMContext):
    ...
    con.close()
    await state.clear()
    await message.answer('✅ Status updated এবং user notification পাঠানো হয়েছে।', reply_markup=main_keyboard())
...
async def x_addadmin(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer('🆔 নতুন Admin Telegram ID পাঠান।')
    await call.answer()
...
async def x_addadmin_msg(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer('❌ Numeric ID দিন।')
    if uid == OWNER_ID:
        return await message.answer(f'ℹ️ {OWNER_ID} Main Owner।')
    con = db()
    con.execute('INSERT OR REPLACE INTO admins(user_id,role,added_at) VALUES(?,?,?)', (uid, 'admin', now_iso()))
    con.commit()
    con.close()
    await state.clear()
    await message.answer(f'✅ Admin {uid} added.', reply_markup=main_keyboard())
...
async def x_rmadmin(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer('🆔 যে Admin remove করবেন তার ID পাঠান।')
    await call.answer()
...
async def x_rmadmin_msg(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer('❌ Numeric ID দিন।')
    if uid == OWNER_ID:
        return await message.answer(f'❌ Main Owner {OWNER_ID} remove করা যাবে না।')
    con = db()
    con.execute('DELETE FROM admins WHERE user_id=?', (uid,))
    con.commit()
    con.close()
    await state.clear()
    await message.answer(f'✅ Admin {uid} removed.', reply_markup=main_keyboard())
