...
async def x_num(message:Message,state:FSMContext):
    ...
    m=(await state.get_data())['method']; setset('bkash_number' if m=='bKash' else 'nagad_number',n); await state.clear(); await message.answer(f'✅ {m} number updated: {n}',reply_markup=main_keyboard())
...
async def x_gw(message:Message,state:FSMContext):
    ...
    m=(await state.get_data())['method']; setset('dep_bkash_gateway' if m=='bKash' else 'dep_nagad_gateway',url); await state.clear(); await message.answer(f'✅ {m} gateway saved.',reply_markup=main_keyboard())
...
async def x_value(message:Message,state:FSMContext):
    ...
    k=(await state.get_data())['key']; setset(k,money(v)); await state.clear(); await message.answer(f'✅ {k} = {money(v)} সেট হয়েছে।',reply_markup=main_keyboard())
...
async def x_fjlink(message:Message,state:FSMContext):
    ...
    d=await state.get_data(); setset('force_join_chat',d['chat']); setset('force_join_link',link); setset('force_join_enabled','1'); await state.clear(); await message.answer('✅ Force Join set ও ON হয়েছে।',reply_markup=main_keyboard())
...
async def x_userid(message:Message,state:FSMContext):
    ...
    r=user_row(uid)
    if not r: await state.clear(); return await message.answer('❌ User পাওয়া যায়নি।',reply_markup=main_keyboard())
    await state.clear()
    kb=InlineKeyboardMarkup(inline_keyboard=[
...
async def x_edit_value(message:Message,state:FSMContext):
    ...
    con=db();con.execute(f'UPDATE users SET {field}=? WHERE user_id=?',(float(v),uid));con.commit();con.close();await state.clear();await message.answer(f'✅ User {uid}: {field} = {money(v)}',reply_markup=main_keyboard())
...
async def x_ubtext(message:Message,state:FSMContext):
    ...
    await state.clear();await message.answer(out,reply_markup=main_keyboard())
...
async def x_reject_reason(message:Message,state:FSMContext):
    ...
    con.commit();con.close();await state.clear();await message.answer('✅ Status updated এবং user notification পাঠানো হয়েছে।',reply_markup=main_keyboard())
...
async def x_addadmin_msg(message:Message,state:FSMContext):
    ...
    if uid==OWNER_ID:return await message.answer(f'ℹ️ {OWNER_ID} Main Owner।')
    con=db();con.execute('INSERT OR REPLACE INTO admins(user_id,role,added_at) VALUES(?,?,?)',(uid,'admin',now_iso()));con.commit();con.close();await state.clear();await message.answer(f'✅ Admin {uid} added.',reply_markup=main_keyboard())
...
async def x_rmadmin_msg(message:Message,state:FSMContext):
    ...
    if uid==OWNER_ID:return await message.answer(f'❌ Main Owner {OWNER_ID} remove করা যাবে না।')
    con=db();con.execute('DELETE FROM admins WHERE user_id=?',(uid,));con.commit();con.close();await state.clear();await message.answer(f'✅ Admin {uid} removed.',reply_markup=main_keyboard())
...
async def x_close(call:CallbackQuery):
    await call.message.answer('🏠 Main Menu',reply_markup=main_keyboard());await call.answer()
