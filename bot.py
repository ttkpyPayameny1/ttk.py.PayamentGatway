...
# =========================
# SINGLE INSTANCE LOCK (duplicate reply prevent)
# =========================
...
def acquire_single_instance_lock():
    ...
    try:
        ...
        handle.close()
        if getattr(e, "errno", None) in (os.errno.EAGAIN, os.errno.EACCES):
            print("=" * 60)
            print("BOT ALREADY RUNNING ELSEWHERE!")
            print("Please close other instance first.")
            print("=" * 60)
            sys.exit(1)
        return
    handle.seek(0)
...
def get_user(message: Message):
    ...
    row = cur.fetchone()
    if not row:
        cur.execute("INSERT INTO users(user_id,name,username,created_at) VALUES(?,?,?,?)",
...
# =========================
# ACCOUNT (FIXED - index unpack)
# =========================
@dp.message(F.text == "👤 একাউন্ট")
async def account(message: Message):
    row = get_user(message)
    # FIX: index-based unpack to avoid "too many values to unpack"
    uid = row[0]
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT plan, expires_at FROM plans WHERE user_id=? AND active=1 ORDER BY expires_at", (uid,))
    active = cur.fetchall()
    con.close()
    vip1 = "সক্রিয় নয়"
    vip2 = "সক্রিয় নয়"
...
async def referral(message: Message):
    ...
    await message.answer(
        ...
        "📌 রেফার করা ব্যক্তি ডিপোজিট করলে, প্রযোজ্য শর্ত অনুযায়ী তার ডিপোজিট থেকে ৫% কমিশন গণনা করা যেতে পারে।",
        parse_mode="Markdown",
        reply_markup=kb
    )
...
async def daily_earnings(message: Message):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT id,plan,amount,activated_at,expires_at,last_claim_date FROM plans WHERE user_id=? AND active=1 ORDER BY id DESC", (uid,))
    plans = cur.fetchall()
    con.close()
    now = datetime.now(timezone.utc)
    ...
    for pid, plan, amount, activated, expires, last_claim in plans:
        ...
        lines.append(f"💎 {plan} — {money(amount)}৳\n📅 মেয়াদ শেষ: {exp.strftime('%Y-%m-%d %H:%M UTC')}\n📊 Demo daily amount: {money(daily)}৳")
    if not valid:
...
async def claim_today(call: CallbackQuery):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT id,plan,amount,last_claim_date FROM plans WHERE user_id=? AND active=1", (uid,))
    rows = cur.fetchall()
    total = Decimal("0")
    claimed = []
    for pid, plan, amount, last_claim in rows:
        ...
        cur.execute("UPDATE plans SET last_claim_date=? WHERE id=?", (today, pid))
...
async def bonus_center(message: Message):
    get_user(message)  # FIX: ensure user exists in DB
    uid = message.from_user.id
    con = db()
    cur = con.cursor()
...
async def bonus_claim(call: CallbackQuery):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT 1 FROM bonuses WHERE user_id=? AND type='daily' AND date(created_at)=date('now')", (uid,))
    if cur.fetchone():
        con.close()
...
async def bonus_history(call: CallbackQuery):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT amount,type,created_at FROM bonuses WHERE user_id=? ORDER BY id DESC LIMIT 5", (call.from_user.id,))
    rows = cur.fetchall()
    con.close()
    if not rows:
...
async def plans_back(call: CallbackQuery):
    await call.message.answer("💎 Available Plans\nএকটি Plan নির্বাচন করুন।", parse_mode="Markdown", reply_markup=plan_keyboard())
    await call.answer()
...
async def buy_plan(call: CallbackQuery):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT COUNT(*) FROM plans WHERE user_id=? AND plan=? AND active=1", (uid, plan))
    ...
    if active_count >= 2:
        ...
        await call.answer()
        return
    cur.execute("SELECT main_balance,deposit_balance FROM users WHERE user_id=?", (uid,))
    row = cur.fetchone()
    ...
    cur.execute("UPDATE users SET main_balance=main_balance-?, deposit_balance=deposit_balance-? WHERE user_id=?",
    ...
    await call.message.answer(
        ...
        "⚠️ Daily amount একটি DEMO/SIMULATION calculation; কোনো নির্দিষ্ট লাভ নিশ্চিত নয়।",
        parse_mode="Markdown"
    )
...
async def transaction_history(message: Message):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT method,amount,status,order_no,created_at FROM deposits WHERE user_id=? ORDER BY id DESC LIMIT 7", (uid,))
    deps = cur.fetchall()
    cur.execute("SELECT method,amount,status,created_at FROM withdrawals WHERE user_id=? ORDER BY id DESC LIMIT 7", (uid,))
    wds = cur.fetchall()
    cur.execute("SELECT amount,claim_date,created_at FROM earnings WHERE user_id=? ORDER BY id DESC LIMIT 7", (uid,))
    earns = cur.fetchall()
    con.close()
...
async def admin_dep_approve(call: CallbackQuery):
    ...
    if status == "Approved":
        ...
        return
    cur.execute("UPDATE deposits SET status='Approved',updated_at=? WHERE id=?", (now_iso(), did))
    cur.execute("UPDATE users SET main_balance=main_balance+?, deposit_balance=deposit_balance+? WHERE user_id=?",
                (amount, amount, uid))
    con.commit()
    con.close()
...
async def admin_dep_reject(call: CallbackQuery):
    ...
    if not row:
        ...
        return
    did, uid, amount, txid, status = row
    cur.execute("UPDATE deposits SET status='Rejected',reason=?,updated_at=? WHERE id=?",
...
async def admin_wd_approve(call: CallbackQuery):
    ...
    con = db()
    cur = con.cursor()
    cur.execute("SELECT user_id,method,account,amount,status FROM withdrawals WHERE id=?", (wid,))
    row = cur.fetchone()
    if not row:
        con.close()
        await call.answer("Request পাওয়া যায়নি।", show_alert=True)
    ...
    if status == "Approved":
        ...
        await call.answer("Already approved.", show_alert=True)
        return
    cur.execute("UPDATE withdrawals SET status='Approved',updated_at=? WHERE id=?", (now_iso(), wid))
    con.commit()
    con.close()
    await bot.send_message(
        uid,
...
async def admin_wd_reject(call: CallbackQuery):
    ...
    if status == "Rejected":
        ...
        return
    cur.execute("UPDATE withdrawals SET status='Rejected',reason=?,updated_at=? WHERE id=?",
...
async def x_pay(call: CallbackQuery):
    if not admin_ok(call.from_user.id):
        return await call.answer('Admin only', show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=a_toggle('Dep bKash', 'dep_bkash_enabled'), callback_data='x:t:dep_bkash_enabled'),
         InlineKeyboardButton(text=a_toggle('Dep Nagad', 'dep_nagad_enabled'), callback_data='x:t:dep_nagad_enabled')],
        [InlineKeyboardButton(text=a_toggle('Wth bKash', 'wth_bkash_enabled'), callback_data='x:t:wth_bkash_enabled'),
         InlineKeyboardButton(text=a_toggle('Wth Nagad', 'wth_nagad_enabled'), callback_data='x:t:wth_nagad_enabled')],
        [InlineKeyboardButton(text=a_toggle('Wth Rocket', 'wth_rocket_enabled'), callback_data='x:t:wth_rocket_enabled'),
         InlineKeyboardButton(text=a_toggle('Wth Upay', 'wth_upay_enabled'), callback_data='x:t:wth_upay_enabled')],
        [InlineKeyboardButton(text='🔄 All Withdraw ON/OFF', callback_data='x:allw')],
        [InlineKeyboardButton(text='📲 Set bKash Number', callback_data='x:num:bKash'),
         InlineKeyboardButton(text='📲 Set Nagad Number', callback_data='x:num:Nagad')],
        [InlineKeyboardButton(text='🔗 Add/Edit bKash Gateway', callback_data='x:gw:bKash'),
         InlineKeyboardButton(text='🔗 Add/Edit Nagad Gateway', callback_data='x:gw:Nagad')],
        [InlineKeyboardButton(text='🗑 Remove bKash Gateway', callback_data='x:rm:bKash'),
         InlineKeyboardButton(text='🗑 Remove Nagad Gateway', callback_data='x:rm:Nagad')],
        [InlineKeyboardButton(text='⬅️ Back', callback_data='x:open')]
    ])
...
async def x_limits(call: CallbackQuery):
    if not admin_ok(call.from_user.id):
        return await call.answer('Admin only', show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f'Min Dep {getset("min_dep")}৳', callback_data='x:v:min_dep'),
         InlineKeyboardButton(text=f'Max Dep {getset("max_dep")}৳', callback_data='x:v:max_dep')],
        [InlineKeyboardButton(text=f'Min Wth {getset("min_wth")}৳', callback_data='x:v:min_wth'),
         InlineKeyboardButton(text=f'Max Wth {getset("max_wth")}৳', callback_data='x:v:max_wth')],
...
async def x_force(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='➕ Set Channel/Group', callback_data='x:fjset'),
...
async def x_userid(message: Message, state: FSMContext):
    ...
    # FIX: named column selection to avoid wrong index mapping
    con = db()
    r = con.execute("""
    ...
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🚫 Ban', callback_data=f'x:ban:{uid}:1'),
         InlineKeyboardButton(text='✅ Unban', callback_data=f'x:ban:{uid}:0')],
        [InlineKeyboardButton(text='⛔ Suspend', callback_data=f'x:sus:{uid}:1'),
         InlineKeyboardButton(text='🟢 Unsuspend', callback_data=f'x:sus:{uid}:0')],
        [InlineKeyboardButton(text='💰 Deposit Balance', callback_data=f'x:edit:{uid}:deposit_balance'),
         InlineKeyboardButton(text='🫂 Referral Balance', callback_data=f'x:edit:{uid}:referral_balance')],
        [InlineKeyboardButton(text='🎁 Bonus Balance', callback_data=f'x:edit:{uid}:bonus_balance'),
         InlineKeyboardButton(text='📝 Task Balance', callback_data=f'x:edit:{uid}:task_balance')],
        [InlineKeyboardButton(text='💵 Main Balance', callback_data=f'x:edit:{uid}:main_balance'),
         InlineKeyboardButton(text='👥 Referral Count', callback_data=f'x:edit:{uid}:referral_count')],
    ...
    await message.answer(
        ...
        f'📝 Task: {money(r[7])}৳\n'
        f'👥 Referrals: {r[8]}\n'
        f'🚫 Ban: {"YES" if r[9] else "NO"}\n'
        f'⛔ Suspend: {"YES" if r[10] else "NO"}',
        parse_mode="Markdown",
        reply_markup=kb
    )
...
async def x_deps(call: CallbackQuery):
    ...
    for did, uid, m, a, t, o, s in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text='✅ Approve', callback_data=f'x:dok:{o}'),
             InlineKeyboardButton(text='❌ Reject', callback_data=f'x:dno:{o}')]
        ])
        await call.message.answer(
            f'📥 #{did}\n👤 {uid}\n💳 {m}\n💰 {money(a)}৳\n📝 `{t}`\n🆔 `{o}`\n📌 {s}',
            parse_mode="Markdown",
...
async def x_wds(call: CallbackQuery):
    ...
    for wid, uid, m, acc, a, s in rows:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text='✅ Approve', callback_data=f'x:wok:{wid}'),
             InlineKeyboardButton(text='❌ Reject', callback_data=f'x:wno:{wid}')]
        ])
        await call.message.answer(
            f'📤 #{wid}\n👤 {uid}\n💳 {m}\n📲 `{acc}`\n💰 {money(a)}৳\n📌 {s}',
            parse_mode="Markdown",
...
async def x_admins(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='➕ Add Admin', callback_data='x:addadmin'),
         InlineKeyboardButton(text='➖ Remove Admin', callback_data='x:rmadmin')],
        [InlineKeyboardButton(text='⬅️ Back', callback_data='x:open')]
    ])
...
async def x_addadmin_msg(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer('❌ Numeric ID দিন।', parse_mode="Markdown")
    if uid == OWNER_ID:
        return await message.answer('ℹ️ 2037461288 Main Owner।', parse_mode="Markdown")
    con = db()
...
async def x_rmadmin_msg(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer('❌ Numeric ID দিন।', parse_mode="Markdown")
    if uid == OWNER_ID:
        return await message.answer('❌ Main Owner 2037461288 remove করা যাবে না।', parse_mode="Markdown")
    con = db()
...
# =========================
# GLOBAL ERROR HANDLER (SAFE - no markdown break)
# =========================
@dp.errors()
async def on_error(event: ErrorEvent):
    logging.error("Unhandled exception: %s", event.exception, exc_info=event.exception)
    try:
        await bot.send_message(ADMIN_ID, f"Bot Error: {type(event.exception).__name__}")
    except Exception:
        pass
